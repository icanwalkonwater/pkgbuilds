...
def expected-problematic-files [] {
  [
    (["inode", "text", "application", "model", "video", "font", "image", "audio", "multipart", "x-content", "message", "chemical", "x-epoc"] | each {|| "/usr/share/mime/" ++ $in} | wrap file | insert type "dir"),
  ] | flatten
}

def remove-expected-problematic-files [] {
  let expected = expected-problematic-files
  for row in $expected {
    let real_type = $row.file | path type
    assert ($real_type != null) $"Problematic file ($row.file) doesn't exist"
    assert ($real_type == $row.type) $"Problematic file ($row.file) is of wrong type: ($real_type), expected ($row.type)"
  }

  # $in | where {|| $in.file $expected}
  $expected
}
...
def "main print-ignore-list" [] {
  remove-expected-problematic-files | explore
}
